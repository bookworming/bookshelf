--[[---------------------------------------------------------------------------------------------------------------------------
 ...
 \ \ \-.  \  \ \ \/\ \  \/_/\_\/_  \ \ \  \ \ \/\ \  \ \ \_\ \  \ \___  \  
  \ \_\\"\_\  \ \_____\   /\_\/\_\  \ \_\  \ \_____\  \ \_____\  \/\_____\ 
   ...
   Made by Team Noxious -- Boxten Sex GUI [Library]
...
function lib.makecoolbutton(text, size, parent, pos, type, textsize, padding, Z, noscale)
	...
	local themes = {
		...
		orang = {
			bg = {{230, 176, 108},{239, 143, 34},{239, 143, 34}},
			stroke = {253,229,201},
			inline = {{253,229,201},{22,22,22}},
		},
		yellow = {
			bg = {{255,240,150},{255,204,69},{255,204,69}},
			stroke = {255,250,200},
			inline = {{255,250,200},{22,22,22}},
		},
		purpl = {
			bg = {{155, 105, 245}, {134, 75, 242}, {134, 75, 242}},
			stroke = {220, 180, 255},
...
function lib.makecoolscrollingframe(size, parent, pos, layoutpadding, Z)
	...
	local function updateBar()
		...
		local idealHeight = ratio * barBackgroundHeight
		local minHeight = 10
		local actualHeight = math.max(idealHeight, minHeight)
		actualHeight = math.min(actualHeight, barBackgroundHeight - 4)
	...
	uis.InputChanged:Connect(function(input)
		...
		if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
			...
			local barBackgroundHeight = scrollbar.AbsoluteSize.Y
			local ratio = view / canvas
			local actualHeight = math.max(ratio * barBackgroundHeight, 10)
			local maxBarTravel = barBackgroundHeight - actualHeight
			local maxScrollPos = canvas - view

			local delta = input.Position.Y - dragStartY
			local scrollDelta = (delta / maxBarTravel) * maxScrollPos
			local newCanvasY = math.clamp(dragStartCanvasY + scrollDelta, 0, maxScrollPos)
			scroll.CanvasPosition = Vector2.new(scroll.CanvasPosition.X, newCanvasY)
		end
	end)
...
function lib.addinputandtoggle(parent, title, description, defaulttext, placeholdertext, callback, default, locked, lockreason, autofill)
	...
	local function updateFrameSize()
		...
		if inputbox then
			inputbox.Position = UDim2.new(0, leftpadding, 1, -30)
		end
	...
	local function onToggleActivated()
		...
		if callback then 
			spwn(callback, inputbox.Text, state.enabled) 
		end
	...
	if default then
		...
		updtoggles()
		if callback then 
			spwn(callback, inputbox.Text, true) 
		end
	end
